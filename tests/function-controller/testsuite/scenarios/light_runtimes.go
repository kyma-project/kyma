	...
	return step.NewSerialTestRunner(logf, "runtime Test",
		...
		step.NewParallelRunner(logf, "fn_tests",
			...
			step.NewSerialTestRunner(nodejs12Logger, "NodeJS12 test",
				...
				teststep.UpdateFunction(nodejs10Logger, nodejs10Cfg.Fn, "Update NodeJS12 Function", runtimes.BasicNodeJSFunction("Hello From updated nodejs12", serverlessv1alpha1.Nodejs12)),
				teststep.NewHTTPCheck(nodejs12Logger, "NodeJS12 pre update simple check through service", nodejs10Cfg.InClusterURL, poll.WithLogger(nodejs12Logger), "Hello From updated nodejs12"),
			),
		),
	), nil
}
