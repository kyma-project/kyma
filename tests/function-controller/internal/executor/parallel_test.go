...
func TestParallelRunner(t *testing.T) {
	...
	steps := executor.NewSerialTestRunner(logf, "Test1",
		testStep{name: "start 0", counter: &idx, logf: logf},
		executor.NewParallelRunner(logf, "Parallel Step",
	...
	runner := executor.NewRunner(executor.WithCleanupDefault(executor.CleanupModeYes), executor.WithLogger(logger))
...
func TestMixedRunners(t *testing.T) {
	...
	steps := executor.NewSerialTestRunner(logf, "Suite",
		testStep{name: "Init Step", counter: &idx, logf: logf},
		executor.NewParallelRunner(logf, "test",
			executor.NewSerialTestRunner(log1, "Test Serial Runner",
			...
			executor.NewSerialTestRunner(log2, "Fault Serial Runner",
			...
			executor.NewSerialTestRunner(log3, "Test Serial Runner",
