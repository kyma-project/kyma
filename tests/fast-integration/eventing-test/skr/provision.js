...
const {
    GatherOptionsFromEnv, WithRuntimeName, WithScenarioName,
    gardener, keb, director
} = require("../../skr-test");
const {eventingScenarioName} = require("../utils");
...
describe("SKR nightly", function () {
    ...
    before(`Fetch last SKR and deprovision if needed`, async function () {
        try {
            let runtime;
            this.options = GatherOptionsFromEnv(
                WithRuntimeName(eventingScenarioName),
                WithScenarioName(eventingScenarioName)
            );
