...
describe("SKR test", function () {
  ...
  const suffix = getEnvOrDefault("FI_SUFFIX", genRandom(4));
  const appName = `app-${suffix}`;
  const runtimeName = `kyma-${suffix}`;
  const scenarioName = `test-${suffix}`;
  const runtimeID = getEnvOrDefault("FI_RUNTIME_ID", uuid.v4());
  const oidc0 = {
    clientID: "abc-xyz",
    groupsClaim: "groups",
  ...
  if (getEnvOrDefault("FI_SKIP_TEST", "false") != "true") {
    ...
    it(`Assure initial OIDC config is part of kubeconfig`, async function () {
      await ensureValidOIDCConfigInCustomerFacingKubeconfig(
        keb,
        runtimeID,
        oidc0
      );
  ...
  if (getEnvOrDefault("FI_SKIP_DEPROVISION", "false") != "true") {
