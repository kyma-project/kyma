...
describe("Kyma with Compass test", async function() {
  ...
  const withCentralApplicationConnectivity = process.env.WITH_CENTRAL_APPLICATION_CONNECTIVITY || false;
  ...
  it("Install Kyma", async function() {
    await installer.installKyma({newEventing: true, withCompass: true, skipComponents, withCentralApplicationConnectivity});
  });
  ...
  it("CommerceMock test fixture should be ready", async function () {
    await ensureCommerceMockWithCompassTestFixture(director, appName, scenarioName,  "mocks", testNS, withCentralApplicationConnectivity);
  });
