...
const (
	...
	publishStatusEndpointURLV1 = "http://event-publish-service.kyma-system:8080/v1/status/ready"
	publishStatusEndpointURLV2 = "http://event-publish-service.kyma-system:8080/v2/status/ready"

	// subscribers
	port               = 9000
	subscriptionNameV1 = "test-subscription-v1"
	subscriptionNameV2 = "test-subscription-v2"
	subscriberNameV1   = "test-core-event-bus-subscriber-v1"
	subscriberNameV2   = "test-core-event-bus-subscriber-v2"
...
func (f *eventBusFlow) createResources() error {
	...
	for _, fn := range []func() error{
		...
		f.checkPublisherStatusV1,
		f.checkPublisherStatusV2,
...
func (f *eventBusFlow) createSubscriber(subscriberName string) error {
	if _, err := f.k8sInterface.AppsV1().Deployments(f.namespace).Get(subscriberName, metav1.GetOptions{}); err != nil {
		...
		if _, err := f.k8sInterface.AppsV1().Deployments(f.namespace).Create(util.NewSubscriberDeploymentWithName(subscriberName, subscriberImage)); err != nil {
			f.log.WithField("error", err).Error("create Subscriber deployment")
			return err
		}
		f.log.Info("create Subscriber service")
		if _, err := f.k8sInterface.CoreV1().Services(f.namespace).Create(util.NewSubscriberServiceWithName(subscriberName)); err != nil {
...
func (f *eventBusFlow) createEventActivation() error {
	...
	return retry.Do(func() error {
		_, err := f.eaInterface.ApplicationconnectorV1alpha1().EventActivations(f.namespace).Create(util.NewEventActivation(eventActivationName, f.namespace, sourceID))
		if err == nil {
			return nil
		}
...
func (f *eventBusFlow) createSubscription(subscriptionName, version string) error {
	...
	_, err := f.subsInterface.EventingV1alpha1().Subscriptions(f.namespace).Create(util.NewSubscription(subscriptionName, f.namespace, eventsEndpoint, eventType, version, sourceID))
	if err != nil {
		if !strings.Contains(err.Error(), "already exists") {
...
func (f *eventBusFlow) checkSubscriberStatus(subscriberName, version string) error {
	...
	endpoint := getSubscriberEventsURL(subscriberName, f.namespace, version)
	return retry.Do(func() error {
...
func (f *eventBusFlow) checkPublisherStatusV1() error {
	return f.checkPublisherStatus(publishStatusEndpointURLV1)
}

func (f *eventBusFlow) checkPublisherStatusV2() error {
	return f.checkPublisherStatus(publishStatusEndpointURLV2)
}

func (f *eventBusFlow) checkPublisherStatus(statusEndpoint string) error {
	f.log.Info("check Publisher status")

	return retry.Do(func() error {
		if err := checkStatus(statusEndpoint); err != nil {
...
func (f *eventBusFlow) publishTestEventV1() error {
	return f.publishTestEvent(version1, eventData, publishEventEndpointURLV1)
}

func (f *eventBusFlow) publishTestEventV2() error {
	return f.publishTestEvent(version2, eventData, publishEventEndpointURLV2)
}
...
func (f *eventBusFlow) publish(version, eventData, publishEventURL string) (*api.PublishResponse, error) {
	payload := fmt.Sprintf(
		`{"source-id": "%s","event-type":"%s","event-type-version":"%s","event-time":"2018-11-02T22:08:41+00:00","data":"%s"}`,
		sourceID, eventType, version, eventData)
...
func (f *eventBusFlow) checkSubscriberReceivedEventV1() error {
	return f.checkSubscriberReceivedEvent(subscriberNameV1, version1, eventData)
}

func (f *eventBusFlow) checkSubscriberReceivedEventV2() error {
	return f.checkSubscriberReceivedEvent(subscriberNameV2, version2, eventData)
}
