...
func printContentsOfNamespace(namespace string) {
	getResourcesCmd := exec.Command("kubectl", "-n", namespace, "get", "deployments,services,replicasets,pods,serviceinstances,servicebindings,servicebindingusages,functions,subscriptions.eventing.kyma-project.io,apis.gateway.kyma-project.io,eventactivation.applicationconnector.kyma-project.io")
	stdoutStderr, err := getResourcesCmd.CombinedOutput()
	output := string(stdoutStderr)
	if err != nil {
		log.Fatal("Unable to get deployments,services,replicasets,pods,serviceinstances,servicebindings,servicebindingusages,functions,subscriptions.eventing.kyma-project.io,apis.gateway.kyma-project.io,eventactivation.applicationconnector.kyma-project.io:\n", output)
	}
