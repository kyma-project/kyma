...
func printContentsOfNamespace(namespace string) {
	getResourcesCmd := exec.Command("kubectl", "-n", namespace, "get", "deploy,svc,rs,po,serviceinstance,servicebinding,servicebindingusage,function,subscriptions.eventing.kyma-project.io,api.gateway.kyma-project.io,eventactivation.applicationconnector.kyma-project.io")
	stdoutStderr, err := getResourcesCmd.CombinedOutput()
	output := string(stdoutStderr)
	if err != nil {
		log.Fatal("Unable to get deploy,svc,rs,po,serviceinstance,servicebinding,servicebindingusage,function,subscriptions.eventing.kyma-project.io,api.gateway.kyma-project.io,eventactivation.applicationconnector.kyma-project.io:\n", output)
	}
