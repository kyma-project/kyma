...
func (s *E2EScenario) Steps(config *rest.Config) ([]step.Step, error) {
	...
	return []step.Step{
		...
		step.Parallel(
			...
			testsuite.NewDeployLambda(s.testID, helpers.LambdaPayload, helpers.LambdaPort, clients.KubelessClientset.KubelessV1beta1().Functions(s.testID), clients.Pods, true),
