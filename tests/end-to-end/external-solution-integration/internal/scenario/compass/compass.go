...
func (s *Scenario) Steps(config *rest.Config) ([]step.Step, error) {
	...
	lambdaEndpoint := helpers.InClusterEndpoint(s.testID, s.testID, helpers.FunctionPort)
	...
	return []step.Step{
		...
		step.Parallel(
			...
			testsuite.NewDeployFunction(s.testID, helpers.FunctionPayload, helpers.FunctionPort, kymaClients.Function, false),
