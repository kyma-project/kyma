...
func NewEventBusTest() (*EventBusTest, error) {
	...
	ebCli, err := eventbusclientSet.NewForConfig(k8sConfig)
	if err != nil {
		return nil, err
	}
...
func (f *eventBusFlow) createSubscription() error {
	...
	return retry.Do(func() error {
		subscription := util.NewSubscription(subscriptionName, f.namespace, subscriberEventEndpointURL, eventType, "v1", srcID)
		if _, err := f.ebInterface.EventingV1alpha1().Subscriptions(f.namespace).Create(subscription); err != nil {
			if !strings.Contains(err.Error(), "already exists") {
