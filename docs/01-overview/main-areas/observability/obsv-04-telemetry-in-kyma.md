...
1. To ship application logs to a new output, create a resource file of kind LogPipeline:
   ...
   An output is a data destination that can be configured by a [Fluent Bit output](https://docs.fluentbit.io/manual/pipeline/outputs) of the relevant type. The LogPipeline supports the two outputs `http` and `custom`. The former pushes the data to the specified http destination while the latter allows the configuration of any destination in the Fluent Bit configuration syntax. An example of the `custom` output is the following:
...
| filters[].custom | string | Actual filter definition in the Fluent Bit syntax.|
...
| output.http.tls.skipCertificateValidation | boolean | Indicates if the validation of certificates should be skipped. Default is `false`. |
...
LogPipeline and LogParser examples can be found in the [samples](https://github.com/kyma-project/kyma/blob/main/components/telemetry-operator/config/samples/telemetry_v1alpha1_logparser.yaml) directory.
