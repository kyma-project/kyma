...
| **spec.typeMatching** | No | Defines the matching type (`standard` or `exact`) for event types. When it is set to `exact`, Eventing does not do any kind of modifications to the provided `spec.types` internally. In case of `standard`, Eventing  modifies the types internally to fulfil the backend requirements. It is set to `standard` unless you specify otherwise. **NOTE:** The `exact` type matching with NATS JetStream is only support for upgradation of Subscription CR from version `v1alpha1` to `v1alpha2`, and it will be deprecated in future. |
