...
1. Navigate to Grafana. It is available under the `https://grafana.{DOMAIN}` address, where `{DOMAIN}` is the domain of your Kyma cluster, such as `https://grafana.kyma.example.com` or `https://grafana.kyma.dev.local/`. To access it from the Console UI, click **Metrics** on the left navigation menu.
