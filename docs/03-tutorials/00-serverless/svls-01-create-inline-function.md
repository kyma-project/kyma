...
3.  In the dialog box, provide the Function's name or generate it by clicking on the **Generate** button. The **Node.js Function** preset is a default one. It means that the **Runtime** is selected to `Node.js 14` and the **Source** code is autogenerated.
