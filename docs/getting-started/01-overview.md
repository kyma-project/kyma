...
<!-- Move the above step to the existing docs (https://kyma-project.io/docs/components/security/#details-iam-kubeconfig-service-get-the-kubeconfig-file-and-configure-the-cli) as they are outdated after recent UI changes. Only link to it here.  -->
...
- Trigger your Function to react to the **order.deliverysent.v1** event from the mock application. Send the event and see if the Function reacts to it by saving its details in the Redis database.
