...
API Gateway is a Kubernetes controller, which operates on APIRule custom resources. To diagnose the problems, inspect the [`status`](../../05-technical-reference/00-custom-resources/apix-01-apirule/#status-codes) field of the APIRule CR:
...
- You are using an access token with proper scopes and it is active:
  ...
  4. Generate a [new access token](../../03-tutorials/00-api-exposure/apix-02-expose-and-secure-service/#register-an-o-auth2-client-and-get-tokens) if needed.
