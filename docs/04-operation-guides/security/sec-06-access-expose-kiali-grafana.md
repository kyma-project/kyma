...
1. Create a new OpenID Connect application for your identity provider. Set the callback URL to the `/oauth2/callback` path of your service, changing the root domain to your cluster domain. For example, use `https://kiali.kyma.example.com/oauth2/callback` for Kiali.
