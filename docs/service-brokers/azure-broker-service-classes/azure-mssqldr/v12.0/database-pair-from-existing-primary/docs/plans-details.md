...
Used to create the secondary database and the failover group based on an existing primary database. It is for taking over an existing database and extending it to a failover group deployment. It creates new secondary database in provisioning and deletes both databases in deprovisioning.

The `azure-sql-12-0-dr-database-pair-from-existing-primary` service provides the following plan names and descriptions:
...
Creates new secondary database.
...
Deletes both databases - primary and secondary.
