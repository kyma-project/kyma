...
>**NOTE:** The monitoring component is available by default in the cluster installation, but disabled in the **Kyma Lite** local installation on Minikube due to highe memory consumption. If you want to run Kyma with monitoring, [install Kyma on a cluster](/root/kyma/#installation-install-kyma-on-a-cluster).
