...
6. The AC reads the CR definition and checks if the Bucket CR is available and its name matches the bucket name referenced in the Asset CR. It also verifies if the Bucket CR is in the `Ready` phase.
