...
>**NOTE:** You can retrieve client certificate by sending a request to get details of a registered service: ``` curl https://gateway.{CLUSTER_DOMAIN}/{APP_NAME}/v1/metadata/services/{YOUR_SERVICE_ID} --cert {CERT_FILE_NAME}.crt --key {KEY_FILE_NAME}.key -k``` Successful call will return response body with base64 encoded client certificate.
