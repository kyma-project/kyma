...
4. Create a lambda that calls the registered service

Create a sample lambda function which sends a request to the registered service with additional path `/uuid`. A successful response returns uuid generated by httpbin.org. To create and register the lambda function in the `production` Environment, run:
```
cat <<EOF | kubectl apply -f -
...
5. Create the Service Binding and the Service Binding Usage to bind the previously create Service Instance to the lambda.
...
6. To expose the lambda outside the cluster create an Api resource:
```
cat <<EOF | kubectl apply -f -
...
7. To verify that everything was setup correctly you can now call the lambda through https:
  - On cluster
    ...
    ```
  - On Minikube
    To be able to access the lambda over https edit `/etc/hosts/` file and map the `my-lambda-production.kyma.local` host to the Minikube ip.
