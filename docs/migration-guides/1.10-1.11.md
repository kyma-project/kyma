...
```
### IP address assignment for LoadBalancer

Following a successful Kyma upgrade, it may happen that the LoadBalancer for the `apiserver-proxy` or `istio-ingressgateway` component receives a new external IP address. In such a case, you must assign this IP address in the DNS server. Follow these steps:
...
Deleting the finalizer makes the garbage collector remove the old Subscriptions and triggers the creation of new Subscriptions. This way, Triggers will receive the `Ready` status. 
