...
Bear in mind that the domain for which you request the certificate is subject to character restrictions. This means that if the domain name you provide in the override exceeds 54 characters, applying it to the cluster will result in the following error:
...
For example, if your cluster domain is `abcdefghij.myproject1.shoot.canary.k8s-hana.ondemand.com`, it already has 56 characters. Together with the `apiserver` prefix, it gives a total of 65 characters which results in an error.
