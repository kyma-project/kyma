...
> **NOTE:** If AddonsConfiguration is marked as `Failed`, all its bundles are not available in the Service Catalog.
...
> **NOTE:** The **namespace** parameter is discarded in case of the ClusterAddonsConfiguration. The rest of parameters are the same.
...
|-------------|---------------|
| Helm Broker |  {Briefly describe the relation between the CR and the given component}. |
