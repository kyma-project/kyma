...
| **authentication.jwt.triggerRules** | **NO** | Specifies the rules for enabling or disabling JWT validation for specific request paths. Refer to Istio [documentation](https://istio.io/docs/reference/config/istio.authentication.v1alpha1/) for usage details. If this parameter value is empty or not provided, JWT validation is enabled for every path. |
