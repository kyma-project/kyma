...
| **authentication.jwt.triggerRule.excludedPaths** | **NO** | Specifies a list of matching expressions that configure resource paths excluded from JWT validation.  See [this](#configuration-of-resources-excluded-from-JWT-validation) section for more details. |
...
- `regex`: matches the path with a regexp as defined by [EDCA-262](https://en.cppreference.com/w/cpp/regex/ecmascript)
Each matching expression takes a string value.
