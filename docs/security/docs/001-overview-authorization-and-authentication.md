...
Out of the box, Kyma comes with its own static user store used by Dex to authenticate users. This solution is designed for use with local Kyma deployments as allow the users to easily create predefined users' credentials by creating secrets with a custom label.
