...
| **kyma-namespace-admin** | `runtimeNamespaceAdmin` (RoleBinding created by [Permission Controller](#details-permission-controller)) | The role that has the same rights as the **kyma-admin** role, except for the write access to [AddonsConfigurations](https://kyma-project.io/docs/master/components/helm-broker#custom-resource-addons-configuration). |

>**NOTE:** The `runtimeDeveloper` group has default binding to the `kyma-essentials` group, which doesn't allow any edit operations, to leave the choice to an administrator to which namespaces developers should have access to. To allow development create a [RoleBinding](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#rolebinding-and-clusterrolebinding) or [ClusterRoleBinding](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#rolebinding-and-clusterrolebinding) to the `kyma-developer` role.
