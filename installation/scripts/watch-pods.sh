...
kubectl create configmap pod-watch-config -n ${WORKING_NAMESPACE} --from-literal="ARGS=-maxWaitingPeriod=10m -ignorePodsPattern=core-azure-broker-docs-*|kyma-release-adder-*|core-service-catalog-etcd-backup-*" >/dev/null
