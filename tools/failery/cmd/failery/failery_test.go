...
func TestParseConfigDefaults(t *testing.T) {
	config := configFromCommandLine("failery")
...
func TestParseConfigFlippingValues(t *testing.T) {
	config := configFromCommandLine("failery -name hi -print -output output -dir dir -recursive -all -inpkg -testonly -case case -note note")
