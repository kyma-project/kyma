...
import (
	...
	"github.com/kyma-project/kyma/tools/failery/failery"
)
...
func main() {
	...
	var osp failery.OutputStreamProvider
	if config.fPrint {
		osp = &failery.StdoutStreamProvider{}
	} else {
		osp = &failery.FileOutputStreamProvider{
	...
	visitor := &failery.GeneratorVisitor{
		InPackage:   config.fIP,
		Note:        config.fNote,
		Osp:         osp,
...
func parseConfigFromArgs(args []string) Config {
	...
	flagSet.BoolVar(&config.fVersion, "version", false, "prints the installed version of failery")
