...
func (s *LogSink) AssertErrorLogged(t *testing.T, errorExpectedMsg string) {
	if !s.wasLogged(t, logrus.ErrorLevel, errorExpectedMsg) {
		t.Errorf("error was not logged, expected: \"%s\"", errorExpectedMsg)
	}
}
