...
func TestReconcileBucketCreationFailed(t *testing.T) {
	...
	instance := fixInitialBucket(name, "", assetstorev1alpha2.BucketPolicy(""))
...
func TestReconcileBucketPolicyUpdateSuccess(t *testing.T) {
	...
	bucket := fixInitialBucket(name, "", assetstorev1alpha2.BucketPolicyReadOnly)
...
func TestReconcileBucketUpdatePolicyFailed(t *testing.T) {
	...
	instance := fixInitialBucket(name, "", assetstorev1alpha2.BucketPolicyWriteOnly)
	expectedRequest := reconcile.Request{NamespacedName: types.NamespacedName{Name: name, Namespace: ""}}
...
func TestReconcileBucketDeletedRemotely(t *testing.T) {
	...
	regionName := string(assetstorev1alpha2.BucketRegionUSEast1)
	bucket := fixInitialBucket(name, regionName, assetstorev1alpha2.BucketPolicyReadOnly)
...
func fixReadyBucket(name string) *assetstorev1alpha2.ClusterBucket {
	return &assetstorev1alpha2.ClusterBucket{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
			Finalizers: []string{
...
func deleteAndExpectSuccess(cfg *testSuite, exp expected, instance *assetstorev1alpha2.ClusterBucket) {
	g := cfg.g
	c := cfg.c
	err := c.Delete(context.TODO(), instance)
	g.Expect(err).NotTo(gomega.HaveOccurred())
