...
func TestCodeMiddleware_Handle(t *testing.T) {
	...
	t.Run("should observe status", func(t *testing.T) {
		// given
		url := "/v1/remoteenvironments/ec-default/tokens"

		collector := &mocks.Collector{}
		collector.On("AddObservation", url, http.MethodPost, float64(testHandlerStatus)).Return()

		codeMiddleware := NewCodeMiddleware(collector)

		router := mux.NewRouter()
		router.Handle(url, codeMiddleware.Handle(getTestHandler()))
		...
		// then
		collector.AssertCalled(t, "AddObservation", url, http.MethodPost, float64(testHandlerStatus))
