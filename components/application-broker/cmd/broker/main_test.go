...
func (ts *testSuite) checkServiceIDs(ids ...string) error {
	req, err := http.NewRequest("GET", fmt.Sprintf("%s/%s/v2/catalog", ts.server.URL, namespace), nil)
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("the response status code msut be 200 but was %d", resp.StatusCode)
	}

	decoder := json.NewDecoder(resp.Body)
	var osbResponse osb.CatalogResponse
	err = decoder.Decode(&osbResponse)
	if err != nil {
		return err
	}
