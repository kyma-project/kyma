// Code generated by mockery v1.0.0. DO NOT EDIT.

package mocks

import authorization "github.com/kyma-project/kyma/components/application-proxy/internal/authorization"
import mock "github.com/stretchr/testify/mock"
import model "github.com/kyma-project/kyma/components/application-proxy/internal/metadata/model"

// StrategyFactory is an autogenerated mock type for the StrategyFactory type
type StrategyFactory struct {
	mock.Mock
}

// Create provides a mock function with given fields: credentials
func (_m *StrategyFactory) Create(credentials *model.Credentials) authorization.Strategy {
	ret := _m.Called(credentials)

	var r0 authorization.Strategy
	if rf, ok := ret.Get(0).(func(*model.Credentials) authorization.Strategy); ok {
		r0 = rf(credentials)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(authorization.Strategy)
		}
	}

	return r0
}
