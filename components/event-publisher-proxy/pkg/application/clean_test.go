...
import (
	...
	eppapplication "github.com/kyma-project/kyma/components/event-publisher-proxy/pkg/application"
...
func TestCleanName(t *testing.T) {
	testCases := []struct {
		...
		{
			givenApplication: applicationtest.NewApplication("alphanumeric0123", map[string]string{eppapplication.TypeLabel: "apptype"}),
			wantName:         "apptype",
		},
		{
			givenApplication: applicationtest.NewApplication("with.!@#none-$%^alphanumeric_&*-characters", map[string]string{eppapplication.TypeLabel: "apptype"}),
			wantName:         "apptype",
		},
		{
			givenApplication: applicationtest.NewApplication("alphanumeric0123", map[string]string{eppapplication.TypeLabel: "apptype=with.!@#none-$%^alphanumeric_&*-characters"}),
			wantName:         "apptypewithnonealphanumericcharacters",
		},
		{
			givenApplication: applicationtest.NewApplication("with.!@#none-$%^alphanumeric_&*-characters", map[string]string{eppapplication.TypeLabel: "apptype=with.!@#none-$%^alphanumeric_&*-characters"}),
			wantName:         "apptypewithnonealphanumericcharacters",
		},
	}

	for _, tc := range testCases {
		if gotName := eppapplication.GetCleanTypeOrName(tc.givenApplication); tc.wantName != gotName {
...
func TestIsCleanName(t *testing.T) {
	...
	for _, tc := range testCases {
		if gotClean := eppapplication.IsCleanName(tc.givenName); tc.wantClean != gotClean {
