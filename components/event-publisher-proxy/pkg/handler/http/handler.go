...
func (h *Handler) publishLegacyEventsAsCE(writer http.ResponseWriter, request *http.Request) {
	...
	h.Logger.Infof("Event dispatched id:[%s] source:[%s] type:[%s] statusCode:[%d] duration:[%s] responseBody:[%s]", event.ID(), event.Source(), event.Type(), statusCode, dispatchTime, respBody)
}
