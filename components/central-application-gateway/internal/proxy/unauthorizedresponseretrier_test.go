...
func TestForbiddenResponseRetrier_CheckResponse(t *testing.T) {
	...
	t.Run("should return response if code is different than 401 and 403", func(t *testing.T) {
		// given
		updateCacheEntryFunction := func(apiIdentifier model.APIIdentifier) (*CacheEntry, apperrors.AppError) {
			return nil, nil
		}
		rr := newUnauthorizedResponseRetrier(model.APIIdentifier{}, &http.Request{}, nil, 10, updateCacheEntryFunction)
	...
	t.Run("should retry if 401 occurred", func(t *testing.T) {
		...
		rr := newUnauthorizedResponseRetrier(apiIdentifier, req, nil, 10, updateCacheEntryFunction)
...
func newUpdateCacheEntryFunction(t *testing.T, url string, strategy authorization.Strategy, csrfTokenStrategy csrf.TokenStrategy) func(identifier model.APIIdentifier) (*CacheEntry, apperrors.AppError) {
	return func(identifier model.APIIdentifier) (*CacheEntry, apperrors.AppError) {
		assert.Equal(t, "app1", identifier.Application)
