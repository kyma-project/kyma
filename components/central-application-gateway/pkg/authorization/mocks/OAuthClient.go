...
// GetTokenMTLS provides a mock function with given fields: clientID, clientSecret, authURL, cert, headers, queryParameters, skipTLSVerification
func (_m *OAuthClient) GetTokenMTLS(clientID string, clientSecret string, authURL string, cert tls.Certificate, headers *map[string][]string, queryParameters *map[string][]string, skipTLSVerification bool) (string, apperrors.AppError) {
	ret := _m.Called(clientID, clientSecret, authURL, cert, headers, queryParameters, skipTLSVerification)

	var r0 string
	if rf, ok := ret.Get(0).(func(string, string, string, tls.Certificate, *map[string][]string, *map[string][]string, bool) string); ok {
		r0 = rf(clientID, clientSecret, authURL, cert, headers, queryParameters, skipTLSVerification)
	} else {
		r0 = ret.Get(0).(string)
	}
	...
	if rf, ok := ret.Get(1).(func(string, string, string, tls.Certificate, *map[string][]string, *map[string][]string, bool) apperrors.AppError); ok {
		r1 = rf(clientID, clientSecret, authURL, cert, headers, queryParameters, skipTLSVerification)
	} else {
		if ret.Get(1) != nil {
			r1 = ret.Get(1).(apperrors.AppError)
		}
