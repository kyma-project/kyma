...
func (src *Subscription) ConvertTo(dstRaw conversion.Hub) error {
	...
	if src.Status.EmsSubscriptionStatus != nil {
		src.setBEBSpecConfig(dst)
	} else { // NATS
		src.setNATSSpecConfig(dst)
	}
	...
	for _, condition := range src.Status.Conditions {
		dst.Status.Conditions = append(dst.Status.Conditions, v1alpha2.ConditionVersion1toVersion2(condition))
	}
...
func (src *Subscription) setBEBSpecConfig(dst *v1alpha2.Subscription) {
...
func (src *Subscription) setNATSSpecConfig(dst *v1alpha2.Subscription) {
