...
type Reconciler struct {
	...
	bebClient        handlers.MessagingBackendHandler
	Domain           string
...
func (r *Reconciler) syncBEBSubscription(subscription *eventingv1alpha1.Subscription, result *ctrl.Result,
	...
	var err error
	if statusChanged, err = r.bebClient.SyncSubscription(subscription, r.eventTypeCleaner, apiRule); err != nil {
...
func (r *Reconciler) deleteBEBSubscription(subscription *eventingv1alpha1.Subscription, logger logr.Logger, ctx context.Context) error {
	...
	if err := r.bebClient.DeleteSubscription(subscription); err != nil {
		return err
	}
