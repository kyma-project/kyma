...
func podSpecEqual(ps1, ps2 *v1.PodSpec) bool {
	if ps1 == ps2 {
		return true
	}
	if ps1 == nil || ps2 == nil {
...
func containerEqual(c1, c2 *v1.Container) bool {
...
func envEqual(e1, e2 []v1.EnvVar) bool {
	if len(e1) != len(e2) {
		return false
	}
EV1:
...
func probeEqual(p1, p2 *v1.Probe) bool {
	if p1 == p2 {
		return true
	}
	if p1 == nil || p2 == nil {
...
func handlerEqual(h1, h2 *v1.Handler) bool {
	if h1 == h2 {
		return true
	}
	if h1 == nil || h2 == nil {
...
func realProto(pr v1.Protocol) v1.Protocol {
	if pr == "" {
...
func eventingBackendStatusEqual(s1, s2 *eventingv1alpha1.EventingBackendStatus) bool {
	...
	if *s1.EventingReady != *s2.EventingReady {
		return false
	}
