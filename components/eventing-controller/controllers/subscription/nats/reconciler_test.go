...
var _ = Describe("NATS subscription reconciler", func() {
	// todo: iterate over these variables:
	id := 1
	eventTypePrefix := reconcilertesting.EventTypePrefix
	// uncomment when used in later tests
	...
	When("NATS server is not reachable and max retries are exceeded", func() {
		...
		itCreateSubscription(ctx, subscription)

		Context("test initial setup", func() {
...
//var (
//	_ = Describe(
//		"NATS Subscription reconciler tests with non-empty eventTypePrefix",
//		testExecutor(
//			reconcilertesting.EventTypePrefix,
...
func itCreateSubscription(ctx context.Context, subscription *eventingv1alpha1.Subscription) {
	It("creates a Subscription", func() {
...
func itStartsReconciler(eventTypePrefix string, natsURL string) context.CancelFunc {
	ctx, cancel := context.WithCancel(context.Background())
	It("starts the reconciler", func() {
		...
		By("passing the k8s manager to the reconciler")
...
// itCreatesSubscriberSvc creates a Service in the k8s cluster. If a custom namespace is used, it will be created as well.
func itCreatesSubscriberSvc(ctx context.Context, svc *v1.Service) {
	It("creates a subscriber svc", func() {
