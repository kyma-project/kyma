...
func testSubscriptionOnNATS(ens *testEnsemble, subscription *eventingv1alpha1.Subscription, subject string, expectations ...gomegatypes.GomegaMatcher) {
	getSubscriptionFromJetStream(ens, subscription, subject).Should(gomega.And(expectations...))
}

func testDeletion(ens *testEnsemble, subscription *eventingv1alpha1.Subscription, subject string) {
	g := ens.g
...
func getSubscriptionFromJetStream(ens *testEnsemble, subscription *eventingv1alpha1.Subscription, subject string) gomega.Assertion {
	g := ens.g
