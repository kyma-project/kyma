	...
	When("Updating the clean event types in the Subscription status", func() {
		It("should mark the Subscription as ready", func() {
			...
			By("Addition of filters to a Subscription", func() {
				publishToSubjects := []eventingv1alpha2.EventType{
					{
						OriginalType: fmt.Sprintf("%s0", reconcilertesting.OrderCreatedEventType),
						CleanType:    fmt.Sprintf("%s0", reconcilertesting.OrderCreatedEventType),
					}, {
						OriginalType: fmt.Sprintf("%s1", reconcilertesting.OrderCreatedEventType),
						CleanType:    fmt.Sprintf("%s1", reconcilertesting.OrderCreatedEventType),
					},
				}
			...
			By("Updating Subscription types", func() {
				cleanEventTypes := []eventingv1alpha2.EventType{
					{
						OriginalType: fmt.Sprintf("%s0alpha", reconcilertesting.OrderCreatedEventType),
						CleanType:    fmt.Sprintf("%s0alpha", reconcilertesting.OrderCreatedEventType),
					}, {
						OriginalType: fmt.Sprintf("%s1alpha", reconcilertesting.OrderCreatedEventType),
						CleanType:    fmt.Sprintf("%s1alpha", reconcilertesting.OrderCreatedEventType),
					},
				}

				By("updating the Subscription types", func() {
					for _, f := range subscription.Spec.Types {
						f = fmt.Sprintf("%salpha", f)
					}
