...
func TestConvertMsgToCE(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			natsMsg: nats.Msg{
				...
				Reply:   "",
				Header:  nil,
				Data:    []byte(natstesting.NewNatsMessagePayload("foo-data", "id", "foosource", eventTime, "fooeventtype")),
				Sub:     nil,
			},
			...
			natsMsg: nats.Msg{
				...
				Reply:   "",
				Header:  nil,
				Data:    []byte(natstesting.NewNatsMessagePayload("foo-data", "", "foosource", eventTime, "fooeventtype")),
				Sub:     nil,
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			gotCE, err := backendnats.ConvertMsgToCE(&tc.natsMsg)
...
func TestSubscription(t *testing.T) {
	...
	cleaner := backendnats.CreateEventTypeCleaner(eventingtesting.EventTypePrefix, eventingtesting.ApplicationNameNotClean, defaultLogger)
...
func TestNatsSubAfterSync_NoChange(t *testing.T) {
	...
	// get cleaned subject
	subject, err := backendnats.GetCleanSubject(sub.Spec.Filter.Filters[0], cleaner)
	...
	for i := 0; i < defaultSubsConfig.MaxInFlightMessages; i++ {
		natsSub := natsBackend.subscriptions[backendnats.CreateKey(sub, subject, i)]
...
func TestNatsSubAfterSync_FiltersChange(t *testing.T) {
	...
	newSubject, err := backendnats.GetCleanSubject(sub.Spec.Filter.Filters[0], cleaner)
	g.Expect(err).ShouldNot(HaveOccurred())
	...
	g.Expect(len(natsBackend.subscriptions)).To(Equal(defaultSubsConfig.MaxInFlightMessages))
	for i := 0; i < defaultSubsConfig.MaxInFlightMessages; i++ {
		natsSub := natsBackend.subscriptions[backendnats.CreateKey(sub, newSubject, i)]
		g.Expect(natsSub).To(Not(BeNil()))
		g.Expect(natsSub.IsValid()).To(BeTrue())
...
func TestNatsSubAfterSync_FilterAdded(t *testing.T) {
	...
	// get cleaned subject
	firstSubject, err := backendnats.GetCleanSubject(sub.Spec.Filter.Filters[0], cleaner)
	g.Expect(err).ShouldNot(HaveOccurred())
	...
	// get new cleaned subject
	secondSubject, err := backendnats.GetCleanSubject(newFilter, cleaner)
	g.Expect(err).ShouldNot(HaveOccurred())
	...
	for i := 0; i < defaultSubsConfig.MaxInFlightMessages; i++ {
		natsSub := natsBackend.subscriptions[backendnats.CreateKey(sub, firstSubject, i)]
		g.Expect(natsSub).To(Not(BeNil()))
		g.Expect(natsSub.IsValid()).To(BeTrue())
...
func TestNatsSubAfterSync_FilterRemoved(t *testing.T) {
	...
	secondSubject, err := backendnats.GetCleanSubject(sub.Spec.Filter.Filters[1], cleaner)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(secondSubject).To(Not(BeEmpty()))
...
func TestNatsSubAfterSync_MultipleSubs(t *testing.T) {
	...
	cleanSubjectSub2, err := backendnats.GetCleanSubject(sub2.Spec.Filter.Filters[0], cleaner)
	g.Expect(err).ShouldNot(HaveOccurred())
	...
	for i := 0; i < defaultSubsConfig.MaxInFlightMessages; i++ {
		natsSub := natsBackend.subscriptions[backendnats.CreateKey(sub2, cleanSubjectSub2, i)]
		g.Expect(natsSub).To(Not(BeNil()))
		g.Expect(natsSub.IsValid()).To(BeTrue())
...
func Test_isNatsSubAssociatedWithKymaSub(t *testing.T) {
	...
	g.Expect(backendnats.IsNatsSubAssociatedWithKymaSub(natsSub1Key, natsSub1, sub1)).To(Equal(true))
	// Should return true because natsSub2 is associated with sub2
	g.Expect(backendnats.IsNatsSubAssociatedWithKymaSub(natsSub2Key, natsSub2, sub2)).To(Equal(true))

	// Should return false because natsSub1 is NOT associated with sub2
	g.Expect(backendnats.IsNatsSubAssociatedWithKymaSub(natsSub1Key, natsSub1, sub2)).To(Equal(false))
	// Should return false because natsSub2 is NOT associated with sub1
	g.Expect(backendnats.IsNatsSubAssociatedWithKymaSub(natsSub2Key, natsSub2, sub1)).To(Equal(false))
}
...
func TestSubscriptionWithMaxInFlightChange(t *testing.T) {
	...
	subject, err := backendnats.GetCleanSubject(filter, cleaner)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(subject).To(Not(BeEmpty()))
...
func TestIsValidSubscription(t *testing.T) {
	...
	cleaner := backendnats.CreateEventTypeCleaner(eventingtesting.EventTypePrefix, eventingtesting.ApplicationName, defaultLogger)
	sub := eventingtesting.NewSubscription("sub", "foo",
	...
	nsn := backendnats.CreateKymaSubscriptionNamespacedName(key, natsSub)
	...
	// restart NATS server
	newNatsServer, _, err := natstesting.StartNATSServer()
	g.Expect(err).To(BeNil())
...
func TestSubscription_NATSServerRestart(t *testing.T) {
	g := NewWithT(t)
	natsServer, natsPort, err := natstesting.StartNATSServer()
	g.Expect(err).To(BeNil())
	defer natsServer.Shutdown()
	defaultLogger := getLogger(g, kymalogger.INFO)
...
func addCleanEventTypesToStatus(sub *eventingv1alpha1.Subscription, cleaner eventtype.Cleaner) {
	sub.Status.CleanEventTypes, _ = backendnats.GetCleanSubjects(sub, cleaner)
}
