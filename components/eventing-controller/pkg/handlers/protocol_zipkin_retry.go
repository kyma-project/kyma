...
func (p *ProtocolZipkin) do(ctx context.Context, req *http.Request) (binding.Message, error) {
...
func (p *ProtocolZipkin) doOnce(req *http.Request) (binding.Message, protocol.Result) {
	resp, err := p.protocol.Client.Do(req)
	if err != nil {
	...
	return cehttp.NewMessage(resp.Header, resp.Body), cehttp.NewResult(resp.StatusCode, "%w", result)
}

func (p *ProtocolZipkin) doWithRetry(ctx context.Context, params *cecontext.RetryParams, req *http.Request) (binding.Message, error) {
	then := time.Now()
	retry := 0
	...
	for {
		...
		if protocol.IsACK(result) {
			return msg, cehttp.NewRetriesResult(result, retry, then, results)
		}
