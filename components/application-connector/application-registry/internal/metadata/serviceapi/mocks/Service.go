...
import (
	...
	applications "github.com/kyma-project/kyma/components/application-connector/application-registry/internal/metadata/applications"
...
func (_m *Service) New(application string, appUID types.UID, id string, displayName string, api *model.API) (*applications.ServiceAPI, apperrors.AppError) {
	ret := _m.Called(application, appUID, id, displayName, api)

	var r0 *applications.ServiceAPI
	if rf, ok := ret.Get(0).(func(string, types.UID, string, string, *model.API) *applications.ServiceAPI); ok {
		r0 = rf(application, appUID, id, displayName, api)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(string, types.UID, string, string, *model.API) apperrors.AppError); ok {
		r1 = rf(application, appUID, id, displayName, api)
	} else {
		if ret.Get(1) != nil {
