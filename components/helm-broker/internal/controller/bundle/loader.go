...
func (l *Loader) Load(in io.Reader) (*internal.Bundle, []*chart.Chart, error) {
	...
	if strings.HasPrefix(cleanPath, l.tmpDir) {
		defer func() {
			...
			l.log.Error(err, fmt.Sprintf("while removing directory %q", unpackedDir))
		}()
	} else {
		defer l.log.Error(
			errors.New("directory not deleted because its name does not resolve to expected path"),
			"while finishing Load method for %q directory", unpackedDir)
	}
...
func (l Loader) discoverPathToHelmChart(baseDir string) (string, error) {
	...
	if len(files) != 0 { // ignoring by design
		l.log.Info("Found files: [%s] in %q bundle directory. All are ignored.", strings.Join(files, ", "), bundleChartDirName)
	}
...
func (l Loader) unpackArchive(baseDir string, in io.Reader) (string, error) {
	...
	defer func() {
		err := unzipped.Close()
		if err != nil {
			l.log.Error(err, "while closing gzip reader")
		}
	}()
...
func (l Loader) createFile(target string, mode int64, r io.Reader) error {
	...
	defer func() {
		err := f.Close()
		if err != nil {
			l.log.Error(err, "while closing file %q", f.Name())
		}
	}()
