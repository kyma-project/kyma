...
func (_m *RequestParametersService) Create(application string, appUID types.UID, bundleID string, requestParameters *model.RequestParameters) (string, apperrors.AppError) {
	ret := _m.Called(application, appUID, bundleID, requestParameters)

	var r0 string
	if rf, ok := ret.Get(0).(func(string, types.UID, string, *model.RequestParameters) string); ok {
		r0 = rf(application, appUID, bundleID, requestParameters)
	} else {
		r0 = ret.Get(0).(string)
	}
	...
	if rf, ok := ret.Get(1).(func(string, types.UID, string, *model.RequestParameters) apperrors.AppError); ok {
		r1 = rf(application, appUID, bundleID, requestParameters)
	} else {
		if ret.Get(1) != nil {
