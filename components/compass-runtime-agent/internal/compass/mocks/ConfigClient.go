...
func (_m *ConfigClient) FetchConfiguration(directorURL string, credentials certificates.Credentials) ([]synchronization.Application, error) {
	ret := _m.Called(directorURL, credentials)

	var r0 []synchronization.Application
	if rf, ok := ret.Get(0).(func(string, certificates.Credentials) []synchronization.Application); ok {
		r0 = rf(directorURL, credentials)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]synchronization.Application)
		}
	}
