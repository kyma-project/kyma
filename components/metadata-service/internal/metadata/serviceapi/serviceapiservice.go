...
func (sas defaultService) New(remoteEnvironment, id string, api *API) (*remoteenv.ServiceAPI, apperrors.AppError) {
	...
	if err != nil {
		return nil, apperrors.Internal("service: creating access service failed, %s", err)
	}
	...
	if sas.oauthCredentialsProvided(api.Credentials) {
		...
		if err != nil {
			return nil, apperrors.Internal("service: creating credentials secret failed, %s", err)
		}
	...
	if err != nil {
		return nil, apperrors.Internal("service: creating Istio resources failed, %s", err)
	}
...
func (sas defaultService) Read(remoteEnvironment string, remoteenvAPI *remoteenv.ServiceAPI) (*API, apperrors.AppError) {
	...
	if remoteenvAPI.OauthUrl != "" && remoteenvAPI.CredentialsSecretName != "" {
		...
		if err != nil {
			return nil, apperrors.Internal("service: reading oauth credentials from %s secret failed, %s",
...
func (sas defaultService) Delete(remoteEnvironment, id string) apperrors.AppError {
	...
	if err != nil {
		return apperrors.Internal("service: deleting access service failed, %s", err)
	}

	err = sas.secretsRepository.Delete(resourceName)
	if err != nil {
		return apperrors.Internal("service: deleting credentials secret failed, %s", err)
	}

	err = sas.istioService.Delete(resourceName)
	if err != nil {
		return apperrors.Internal("service: deleting Istio resources failed, %s", err)
	}
...
func (sas defaultService) Update(remoteEnvironment, id string, api *API) (*remoteenv.ServiceAPI, apperrors.AppError) {
	...
	if sas.oauthCredentialsProvided(api.Credentials) {
		...
		if err != nil {
			return nil, apperrors.Internal("service: updating credentials secret failed, %s", err)
		}
		serviceAPI.OauthUrl = api.Credentials.Oauth.URL
		serviceAPI.CredentialsSecretName = resourceName
	} else {
		err := sas.secretsRepository.Delete(resourceName)
		if err != nil {
			return nil, apperrors.Internal("service: deleting credentials secret failed, %s", err)
		}
	}

	err = sas.istioService.Upsert(remoteEnvironment, id, resourceName)
	if err != nil {
		return nil, apperrors.Internal("service: updating Istio resources failed, %s", err)
	}
