...
func (r *repository) Create(remoteEnvironment string, service Service) apperrors.AppError {
	...
	if e != nil {
		return apperrors.Internal(fmt.Sprintf("repository: creating service failed, %s", e.Error()))
	}
...
func (r *repository) Get(remoteEnvironment, id string) (Service, apperrors.AppError) {
	...
	return Service{}, apperrors.NotFound(fmt.Sprintf("repository: service with ID %s not found", id))
}
...
func (r *repository) Update(remoteEnvironment string, service Service) apperrors.AppError {
	...
	_, e := r.reManager.Update(re)
	if e != nil {
		return apperrors.Internal(fmt.Sprintf("repository: updating service failed, %s", e.Error()))
	}
...
func (r *repository) Delete(remoteEnvironment, id string) apperrors.AppError {
	...
	_, e := r.reManager.Update(re)
	if e != nil {
		return apperrors.Internal(fmt.Sprintf("repository: deleting service failed, %s", e.Error()))
	}
...
func (r *repository) getRemoteEnvironment(remoteEnvironment string) (*v1alpha1.RemoteEnvironment, apperrors.AppError) {
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			message := fmt.Sprintf("repository: Remote Environment %s not found", remoteEnvironment)
			return nil, apperrors.Internal(message)
		}

		message := fmt.Sprintf("repository: getting Remote Environment %s failed, %s", remoteEnvironment, err.Error())
		return nil, apperrors.Internal(message)
	}
