...
func (sds *serviceDefinitionService) Update(remoteEnvironment, id string, serviceDef *ServiceDefinition) apperrors.AppError {
	...
	if err != nil {
		if err.Code() != apperrors.CodeNotFound {
			return apperrors.NotFound("failed to get service before update, %existingSvc", err)
		}
		return apperrors.Internal("failed to read service, %existingSvc", err)
	}
	...
	if !apiDefined(serviceDef) {
		...
		if err != nil {
			return apperrors.Internal("failed to delete API, %existingSvc", err)
		}
	} else {
		service.API, err = sds.serviceAPIService.Update(remoteEnvironment, id, serviceDef.Api)
		if err != nil {
			return apperrors.Internal("failed to update API, %existingSvc", err)
		}

		serviceDef.Api.Spec, err = modifyAPISpec(serviceDef.Api.Spec, service.API.GatewayURL)
		if err != nil {
			return apperrors.Internal("failed to modify API spec, %existingSvc", err)
		}
	}

	err = sds.insertSpecs(id, serviceDef.Documentation, serviceDef.Api, serviceDef.Events)
	if err != nil {
		return apperrors.Internal("failed to insert specification to Minio, %existingSvc", err)
	}

	err = sds.remoteEnvironmentRepository.Update(remoteEnvironment, *service)
	if err != nil {
		return apperrors.Internal("failed to update service in RE repository, %existingSvc")
	}
