...
func (m *accessServiceManager) Create(remoteEnvironment, serviceId, serviceName string) apperrors.AppError {
	...
	if err != nil {
		return apperrors.Internal("access service manager: creating service failed, %s", err)
	}
	return nil
}
...
func (m *accessServiceManager) Upsert(remoteEnvironment, serviceId, serviceName string) apperrors.AppError {
	...
	if err != nil {
		...
		return apperrors.Internal("access service manager: upserting service failed, %s", err)
	}
	return nil
}
...
func (m *accessServiceManager) Delete(serviceName string) apperrors.AppError {
	...
	if err != nil {
		if !k8serrors.IsNotFound(err) {
			return apperrors.Internal("access service manager: deleting service failed, %s", err)
		}
	}
	return nil
}
