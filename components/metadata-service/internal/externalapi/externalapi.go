...
func NewHandler(handler MetadataHandler, middlewares []mux.MiddlewareFunc) http.Handler {
	...
	router.Path("/{remoteEnvironment}/v1/metadata").Handler(http.RedirectHandler("/v1/metadataapi.yaml", http.StatusMovedPermanently)).Methods(http.MethodGet)
	router.Path("/{remoteEnvironment}/v1/metadataapi.yaml").Handler(http.RedirectHandler("/v1/metadataapi.yaml", http.StatusMovedPermanently)).Methods(http.MethodGet)

	router.Path("/v1/metadataapi.yaml").Handler(NewStaticFileHandler(apiSpecPath)).Methods(http.MethodGet)
