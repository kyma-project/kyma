// Code generated by mockery v1.0.0. DO NOT EDIT.
package mocks

import mock "github.com/stretchr/testify/mock"
import v1alpha1 "github.com/kyma-project/kyma/components/connectivity-certs-controller/pkg/apis/applicationconnector/v1alpha1"

// CentralConnectionManager is an autogenerated mock type for the CentralConnectionManager type
type CentralConnectionManager struct {
	mock.Mock
}

// Create provides a mock function with given fields: _a0
func (_m *CentralConnectionManager) Create(_a0 *v1alpha1.CentralConnection) (*v1alpha1.CentralConnection, error) {
	ret := _m.Called(_a0)

	var r0 *v1alpha1.CentralConnection
	if rf, ok := ret.Get(0).(func(*v1alpha1.CentralConnection) *v1alpha1.CentralConnection); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*v1alpha1.CentralConnection)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(*v1alpha1.CentralConnection) error); ok {
		r1 = rf(_a0)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
