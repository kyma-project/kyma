...
func TestPluggableResolver(t *testing.T) {
	pluggable, err := apigateway.New(&rest.Config{}, informerResyncPeriod)
...
func checkExportedFields(t *testing.T, resolver *apigateway.PluggableResolver, enabled bool) {
	assert.NotNil(t, resolver.Resolver)
	namespace := "default"

	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Second)
	defer cancel()
	val, err := resolver.Resolver.APIRulesQuery(ctx, namespace, nil, nil)
	if enabled {
		require.NoError(t, err)
	} else {
		require.Error(t, err)
