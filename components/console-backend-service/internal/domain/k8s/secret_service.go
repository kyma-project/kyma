...
func (svc secretService) Update(name, namespace string, update v1.Secret) (*v1.Secret, error) {
	...
	updated, err := svc.client.Secrets(namespace).Update(context.TODO(), &update, metav1.UpdateOptions{})
	if err != nil {
		return nil, err
	}
...
func (svc secretService) Delete(name, namespace string) error {
	return svc.client.Secrets(namespace).Delete(context.TODO(), name, metav1.DeleteOptions{})
}
