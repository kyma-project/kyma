...
func (svc *podService) Update(name, namespace string, update v1.Pod) (*v1.Pod, error) {
	...
	updated, err := svc.client.Pods(namespace).Update(context.TODO(), &update, metav1.UpdateOptions{})
	if err != nil {
		return nil, err
	}
...
func (svc *podService) Delete(name, namespace string) error {
	return svc.client.Pods(namespace).Delete(context.TODO(), name, metav1.DeleteOptions{})
}
