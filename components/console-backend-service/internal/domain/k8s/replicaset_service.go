...
func (svc *replicaSetService) Update(name, namespace string, update apps.ReplicaSet) (*apps.ReplicaSet, error) {
	...
	updated, err := svc.client.ReplicaSets(namespace).Update(context.TODO(), &update, metav1.UpdateOptions{})
	if err != nil {
		return nil, err
	}
...
func (svc *replicaSetService) Delete(name, namespace string) error {
	return svc.client.ReplicaSets(namespace).Delete(context.TODO(), name, metav1.DeleteOptions{})
}
