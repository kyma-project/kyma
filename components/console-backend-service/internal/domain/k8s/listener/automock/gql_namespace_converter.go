...
func (_m *gqlNamespaceConverter) ToGQL(in types.NamespaceWithAdditionalData) (*gqlschema.Namespace, error) {
	ret := _m.Called(in)

	var r0 *gqlschema.Namespace
	if rf, ok := ret.Get(0).(func(types.NamespaceWithAdditionalData) *gqlschema.Namespace); ok {
		r0 = rf(in)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(types.NamespaceWithAdditionalData) error); ok {
		r1 = rf(in)
	} else {
		r1 = ret.Error(1)
	}
