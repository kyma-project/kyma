...
func NewController(client client.Client, log logr.Logger, config configwatcher.Config, resourceType ResourceType, services *configwatcher.Services) *Reconciler {
	return &Reconciler{
		Client:       client,
		Log:          log,
		config:       config,
...
func (r *Reconciler) reconcileNamespace(req ctrl.Request) (ctrl.Result, error) {
	...
	return ctrl.Result{
		RequeueAfter: r.config.NamespaceRequeueDuration,
	}, nil
}
