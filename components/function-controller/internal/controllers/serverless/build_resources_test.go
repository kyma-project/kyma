	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for key, value := range got.Spec.Selector.MatchLabels {
				...
				g.Expect(got.Spec.Template.Spec.Containers[0].LivenessProbe.SuccessThreshold).To(gomega.Equal(1), "documentation states that this value has to be set to 1")
				g.Expect(got.Spec.Template.Spec.Containers[0].StartupProbe.SuccessThreshold).To(gomega.Equal(1), "documentation states that this value has to be set to 1")
			}
		})
	}
}
