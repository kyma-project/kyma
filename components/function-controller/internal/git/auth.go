...
func (a *AuthOptions) toKeyAuth() (*gitssh.PublicKeys, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "while creating public keys authentication method, password: %s", password) // TODO: remove after tests
	}

	auth := gitssh.PublicKeys{
		User:   "git",
		Signer: signer,
		HostKeyCallbackHelper: gitssh.HostKeyCallbackHelper{func(hostname string, remote net.Addr, key ssh.PublicKey) error {
			return nil
		}},
	}
