...
func (publisher *DefaultKnativePublisher) Publish(knativeLib *knative.KnativeLib, channelName *string,
	...
	if err != nil {
		log.Printf("cannot find the knative channel '%v' in namespace '%v' with error: '%v'", *channelName,
			*namespace, err)
	...
	if err != nil {
		log.Printf("failed to send message to the knative channel '%v' in namespace '%v' with error: '%v'",
			*channelName, *namespace, err)
		return api.ErrorResponseInternalServer(), FAILED
	}
