...
To install the Compass with GCP managed Postgres database, set the `database.embedded.enabled` value to `false` inside `./chart/compass/values.yaml` file, and fill those values:
