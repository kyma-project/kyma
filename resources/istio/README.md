...
1. If you are enabling `kiali`, make sure you have enabled `prometheus-operator` and `monitoring` components. Kiali installer will generate the secret with the username `admin` and an autogenerated passphrase, you will need these to login to Kiali UI. To get the password use:
```
