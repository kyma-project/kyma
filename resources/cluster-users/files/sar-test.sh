...
if [ $OIDC_PROVIDER == "hydra" ]; then
  KYMA_VERSION="2"
fi
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
KYMA_PROJECT_DIR=$(readlink -f "${DIR}/../../..")
...
function getConfigFileHydra() {
  ...
  token=$(go run main.go \
    ...
    -oidcClientRedirectUrl "$oidc_client_redirect_url" \
    -oidcProviderFqdn "$oidc_provider_fqdn")
  popd
  ...
  kubectl config set-context --current --user "${EMAIL}"
  cat $KUBECONFIG
}
...
function runTests() {
    ...
    OIDC_PROVIDER=${OIDC_PROVIDER} EMAIL=${VIEW_EMAIL} PASSWORD=${VIEW_PASSWORD} getConfigFile
    export KUBECONFIG="/tmp/kubeconfig"
  ...
  OIDC_PROVIDER=${OIDC_PROVIDER} EMAIL=${NAMESPACE_ADMIN_EMAIL} PASSWORD=${NAMESPACE_ADMIN_PASSWORD} getConfigFile
  export KUBECONFIG="/tmp/kubeconfig"
    ...
    #  Disabled in kyma 2 because of lack of permission controller
    if [ $KYMA_VERSION != "2" ]; then
  ...
  OIDC_PROVIDER=${OIDC_PROVIDER} EMAIL=${DEVELOPER_EMAIL} PASSWORD=${DEVELOPER_PASSWORD} getConfigFile
  export KUBECONFIG="/tmp/kubeconfig"
